...
| 托管方式 | 3 天后过期 | 订阅有效期间持续托管 | 订阅有效期间持续托管 |
| 当前托管保障 | 3 天 | 1 个月，续订自动延长 | 12 个月，续订自动延长 |
...
Free 默认启用；Pro 仅在 Stripe subscription `active` 时提供新的商业权益。现有网站在已付账期结束后还有 3 天托管宽限；有效订阅续费后自动延长，无需重新部署。不提供 trial。Stripe Price 保留原价 $9/月、$84/年，Checkout 自动应用符合条件的优惠：默认新用户 10% off forever；BR/PT 葡语投放与流失/犹豫触发场景使用限时 40% 优惠；符合 legacy 资格的老月付用户升级年付时继续获得 40% off forever。
